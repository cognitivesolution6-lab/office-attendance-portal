...
// =============================================
// APNE EMPLOYEES YAHAN ADD KAREIN
// =============================================
const EMPLOYEES = [
...
// SETTINGS
// =============================================
const DAILY_DEDUCTION = 200; // Rs. per late day
...
export default function AttendancePortal() {
      ...
      setAttendance((prev) => {
        ...
        return {
          ...
          [todayKey]: {
            ...
            ...(type === "in"
              ? { checkIn: displayTime, checkInRaw: timeStr, checkInLoc: loc, isLate }
              : { checkOut: displayTime, checkOutLoc: loc }),
            status: existing.status || (isLate ? "late" : "present"),
          },
        };
      });
      if (loc) setLocationStatus(loc);
      showNotif(`✅ ${type === "in" ? "Check-In" : "Check-Out"} ho gaye! ${displayTime}${loc ? " — Location capture ho gayi" : ""}`);
    };

    navigator.geolocation.getCurrentPosition(
      (pos) => save({ lat: pos.coords.latitude.toFixed(5), lng: pos.coords.longitude.toFixed(5) }),
      ()    => { showNotif("Location access nahi mili — attendance phir bhi mark ho rahi hai", "warning"); save(null); }
    );
  };
  ...
  const applyLeave = () => {
    if (!leaveForm.from || !leaveForm.to || !leaveForm.reason) {
      showNotif("Sab fields bharein!", "error");
      return;
    }
    setLeaves((prev) => [
      ...prev,
      { ...leaveForm, userId: currentUser.id, status: "pending", id: Date.now(), name: currentUser.name },
    ]);
  ...
  const getMonthStats = (userId) => {
    ...
    return { present, late, absent, deduction: late * DAILY_DEDUCTION, total: recs.length };
  };
  ...
  const getDayStatus = (date) => {
    ...
    if (!rec) return (date.getDay() === 0 || date.getDay() === 6) ? "holiday" : null;
    return rec.status;
  };
      ...
      <div style={{ minHeight: "100vh", background: "linear-gradient(135deg,#0f172a 0%,#1e3a5f 50%,#0f4c81 100%)", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'Segoe UI',sans-serif" }}>
        <div style={{ background: "rgba(255,255,255,0.05)", backdropFilter: "blur(20px)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 24, padding: "48px 40px", width: 380, boxShadow: "0 25px 50px rgba(0,0,0,0.5)" }}>
          ...
          {EMPLOYEES.map((emp) => (
            <button
              key={emp.id}
              onClick={() => { setCurrentUser(emp); setActiveTab("dashboard"); }}
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#f0f4f8", fontFamily: "'Segoe UI',sans-serif" }}>
      ...
      {/* ---- Notification ---- */}
      {notification && (
        <div style={{ position: "fixed", top: 20, right: 20, zIndex: 9999, background: notification.type === "error" ? "#ef4444" : notification.type === "warning" ? "#f59e0b" : "#10b981", color: "#fff", padding: "14px 20px", borderRadius: 12, fontSize: 14, fontWeight: 600, boxShadow: "0 8px 24px rgba(0,0,0,0.2)", maxWidth: 320 }}>
          {notification.msg}
        </div>
      )}
        ...
        <div style={{ maxWidth: 900, margin: "0 auto", display: "flex", gap: 4 }}>
          {tabs.map((t) => (
            <button
              ...
              style={{ background: activeTab === t.id ? "rgba(255,255,255,0.15)" : "transparent", border: "none", borderBottom: activeTab === t.id ? "2px solid #60a5fa" : "2px solid transparent", color: activeTab === t.id ? "#fff" : "#93c5fd", padding: "12px 16px", cursor: "pointer", fontSize: 13, fontWeight: 600, borderRadius: "8px 8px 0 0", display: "flex", alignItems: "center", gap: 6 }}
            >
              {t.icon} {t.label}
            </button>
          ))}
        ...
        {activeTab === "dashboard" && (
          <div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 16, marginBottom: 24 }}>
              {[
                { label: "Present Days",    value: stats.present,              icon: "✅", color: "#10b981" },
                { label: "Late Days",       value: stats.late,                 icon: "⏰", color: "#f59e0b" },
                { label: "Absent Days",     value: stats.absent,               icon: "❌", color: "#ef4444" },
                { label: "Late Deduction",  value: `Rs. ${stats.deduction}`,   icon: "💸", color: "#6366f1" },
              ].map((s) => (
            ...
            <div style={{ background: "#fff", borderRadius: 16, padding: 24, boxShadow: "0 2px 12px rgba(0,0,0,0.06)" }}>
              ...
              {todayActivities.length === 0
                ? <p style={{ color: "#94a3b8", fontSize: 14 }}>Abhi koi activity nahi daali</p>
            ...
            <div style={{ background: "#fff", borderRadius: 20, padding: 32, boxShadow: "0 2px 12px rgba(0,0,0,0.06)", marginBottom: 20 }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 24 }}>
                ...
                <button
                  ...
                  style={{ padding: "20px", borderRadius: 16, border: "none", cursor: todayAtt?.checkIn ? "not-allowed" : "pointer", background: todayAtt?.checkIn ? "#d1fae5" : "linear-gradient(135deg,#10b981,#059669)", color: todayAtt?.checkIn ? "#059669" : "#fff", fontSize: 16, fontWeight: 700, boxShadow: todayAtt?.checkIn ? "none" : "0 4px 20px rgba(16,185,129,0.4)", transition: "all 0.2s" }}
                >
                ...
                <button
                  ...
                  style={{ padding: "20px", borderRadius: 16, border: "none", cursor: (!todayAtt?.checkIn || todayAtt?.checkOut) ? "not-allowed" : "pointer", background: todayAtt?.checkOut ? "#dbeafe" : todayAtt?.checkIn ? "linear-gradient(135deg,#3b82f6,#1d4ed8)" : "#f1f5f9", color: todayAtt?.checkOut ? "#1d4ed8" : todayAtt?.checkIn ? "#fff" : "#94a3b8", fontSize: 16, fontWeight: 700, boxShadow: (todayAtt?.checkIn && !todayAtt?.checkOut) ? "0 4px 20px rgba(59,130,246,0.4)" : "none", transition: "all 0.2s" }}
                >
              ...
              {todayAtt?.isLate && (
                <div style={{ background: "#fef3c7", border: "1px solid #fcd34d", borderRadius: 12, padding: 16, marginBottom: 16 }}>
                  ...
                  <p style={{ color: "#92400e", margin: "4px 0 0", fontSize: 13 }}>Aap late aaye hain. Deduction: <strong>Rs. {DAILY_DEDUCTION}</strong></p>
                </div>
              )}
              ...
              {locationStatus && (
                <div style={{ background: "#f0fdf4", border: "1px solid #bbf7d0", borderRadius: 12, padding: 14 }}>
                  ...
                  <a href={`https://maps.google.com/?q=${locationStatus.lat},${locationStatus.lng}`} target="_blank" rel="noreferrer" style={{ fontSize: 12, color: "#16a34a", textDecoration: "none", display: "inline-block", marginTop: 4 }}>
                    🗺️ Google Maps mein dekhai →
                  </a>
                </div>
              )}
            </div>
        ...
        {activeTab === "leave" && (
          <div>
            ...
            <div style={{ background: "#fff", borderRadius: 20, padding: 24, boxShadow: "0 2px 12px rgba(0,0,0,0.06)" }}>
              ...
              {myLeaves.length === 0
                ? <p style={{ color: "#94a3b8" }}>Koi leave apply nahi ki abhi tak</p>
                : myLeaves.map((l) => (
                    <div key={l.id} style={{ padding: "14px 0", borderBottom: "1px solid #f1f5f9", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                      ...
                      <span style={{ padding: "4px 12px", borderRadius: 20, fontSize: 12, fontWeight: 700, background: l.status === "approved" ? "#d1fae5" : l.status === "rejected" ? "#fee2e2" : "#fef3c7", color: l.status === "approved" ? "#059669" : l.status === "rejected" ? "#dc2626" : "#d97706" }}>
        ...
        {activeTab === "monthly" && (
          <div>
            ...
            <div style={{ background: "#fff", borderRadius: 16, padding: "16px 20px", boxShadow: "0 2px 12px rgba(0,0,0,0.06)", marginBottom: 16, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              <button
                onClick={() => setViewMonth((p) => { const d = new Date(p.year, p.month - 1); return { year: d.getFullYear(), month: d.getMonth() }; })}
                style={{ border: "none", background: "#f1f5f9", borderRadius: 8, padding: "8px 14px", cursor: "pointer", fontWeight: 700 }}
              >← Pehla</button>
            ...
            <div style={{ background: "#fff", borderRadius: 20, padding: 20, boxShadow: "0 2px 12px rgba(0,0,0,0.06)" }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(7,1fr)", gap: 3 }}>
                ...
                {monthDays.map((day) => {
                  ...
                  const bg  = status ? STATUS_COLORS[status] : isWeekend ? "#f1f5f9" : "#f8fafc";
                  const col = status ? "#fff" : isWeekend ? "#94a3b8" : "#475569";
                  return (
                    <div key={day.toISOString()} style={{ borderRadius: 8, padding: "8px 4px", textAlign: "center", background: bg, color: col, fontSize: 13, fontWeight: isToday ? 800 : 500, border: isToday ? "2px solid #3b82f6" : "2px solid transparent", minHeight: 34, display: "flex", alignItems: "center", justifyContent: "center" }}>
                      {day.getDate()}
                    </div>
                  );
                })}
            ...
            <div style={{ background: "#fff", borderRadius: 16, padding: 20, boxShadow: "0 2px 12px rgba(0,0,0,0.06)", marginTop: 16 }}>
              ...
              {getUserAttendance(currentUser.id, viewMonth.year, viewMonth.month).length === 0
                ...
                : getUserAttendance(currentUser.id, viewMonth.year, viewMonth.month).map((rec, i) => (
                    <div key={i} style={{ display: "grid", gridTemplateColumns: "110px 1fr 1fr 80px 90px", gap: 12, padding: "10px 0", borderBottom: "1px solid #f1f5f9", fontSize: 13, alignItems: "center" }}>
                      ...
                      <div style={{ color: rec.isLate ? "#ef4444" : "#94a3b8", fontSize: 12 }}>
                        {rec.isLate ? `-Rs.${DAILY_DEDUCTION}` : "—"}
                      </div>
                    </div>
                  ))
              }
